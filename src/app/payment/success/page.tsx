...
export default function PaymentSuccess() {
  ...
  const processPaymentSuccess = async (sessionId: string) => {
    try {
      ...
      const response = await fetch('/api/stripe/process-payment-success', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        console.log('Payment processed successfully:', data.message)
      } else {
        console.error('Failed to process payment success:', data.error)
      }
    } catch (error) {
                ...
                <div className="backdrop-blur-sm bg-black/20 border border-white/10 rounded-2xl p-6 mb-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="text-white/70 text-sm">{paymentDetails.searchData.medication} {paymentDetails.searchData.dosage} • {paymentDetails.pharmacyCount} pharmacies</div>
                    </div>

                    <div className="space-y-2 pt-4 border-t border-white/10">
                      <div className="flex justify-between">
                        <span className="text-white/70">Subtotal</span>
                        <span className="text-white">${paymentDetails.amount}.00</span>
                      </div>
                      <div className="flex justify-between font-semibold">
                        <span className="text-white">Total</span>
                        <span className="text-white">${paymentDetails.amount}.00 USD</span>
                      </div>
                    </div>
